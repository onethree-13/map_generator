...
class TagManagementTab:
    ...
    def render(self):
        ...
        data_items = self.data_manager.get_saved_json().get("data", [])
    ...
    def _handle_tag_refresh(self, all_tags):
        ...
        if 'last_tag_count' not in st.session_state:
            ...
            if 'selected_tags' in st.session_state:
                # 只保留仍然存在的标签
                st.session_state.selected_tags = st.session_state.selected_tags.intersection(set(all_tags))
    ...
    def _render_location_selection(self, data_items):
        ...
        with location_action_col1:
            if st.button("✅ 全选地点", key="select_all_locations"):
                st.session_state.selected_locations = set(range(len(data_items)))
    ...
    def _render_tag_selection(self, all_tags):
        ...
        with tag_action_col3:
            # 添加新标签
            new_tag = st.text_input("新标签", placeholder="输入新标签", key="new_tag_input", label_visibility="collapsed")
    ...
    def _render_selection_status(self, data_items):
        ...
        with status_col1:
            selected_location_names = [data_items[i].get("name", f"地点 {i+1}") for i in st.session_state.selected_locations]
            st.info(f"已选择 {len(st.session_state.selected_locations)} 个地点: {', '.join(selected_location_names) if selected_location_names else '无'}")
        with status_col2:
            st.info(f"已选择 {len(st.session_state.selected_tags)} 个标签: {', '.join(st.session_state.selected_tags) if st.session_state.selected_tags else '无'}")
    ...
    def _execute_add_tags(self):
        ...
        # 增加表格刷新计数器
        st.session_state.table_refresh_counter = getattr(st.session_state, 'table_refresh_counter', 0) + 1
        
        # 显示成功消息
        st.success(f"✅ 已为 {len(st.session_state.selected_locations)} 个地点添加 {len(st.session_state.selected_tags)} 个标签")
    ...
    def _execute_overwrite_tags(self):
        ...
        for index in st.session_state.selected_locations:
            if 0 <= index < len(data_items):
                data_items[index]["tags"] = list(st.session_state.selected_tags)
    ...
    def _render_ai_tag_editing(self):
        ...
        if st.button("🚀 执行AI标签编辑", type="primary", disabled=not ai_instruction.strip()):
    ...
    def _execute_ai_tag_editing(self, ai_instruction):
        ...
        try:
            with st.spinner("AI正在处理标签编辑指令..."):
                # 构建专门的标签编辑prompt
                current_data = self.data_manager.get_saved_json()
...
4. 不要添加任何解释，只返回JSON"""
                ...
                # 确保数据更新到data_manager
                self.data_manager.set_saved_json(edited_data)
    ...
    def _render_tag_editing_table(self):
        ...
        current_data_items = self.data_manager.get_saved_json().get("data", [])
        ...
        # 显示数据统计
        total_locations = len(current_data_items)
        locations_with_tags = sum(1 for item in current_data_items if item.get("tags"))
        st.info(f"📊 当前共有 {total_locations} 个地点，其中 {locations_with_tags} 个地点有标签")
        
        # 生成基于数据内容的唯一key，确保数据变化时表格能刷新
        if 'table_refresh_counter' not in st.session_state:
            st.session_state.table_refresh_counter = 0
        
        table_content_hash = hash(str([(item.get("name", ""), ",".join(item.get("tags", []))) for item in current_data_items]) + str(st.session_state.table_refresh_counter))
        ...
        # 应用表格修改
        if st.button("💾 应用表格修改", type="primary"):
    ...
    def _apply_table_modifications(self, edited_table):
        ...
        try:
            # 使用最新的data_items进行修改
            saved_json = self.data_manager.get_saved_json()
            latest_data_items = saved_json.get("data", [])
